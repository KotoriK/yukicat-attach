...
import React, { CSSProperties, forwardRef, MutableRefObject, RefObject, useCallback, useState } from 'react'
...
function PageView({ path: path_raw, raw }: { path: string, raw: string }) {
    ...
    return <>
        <span onClick={handleClick} ref={setRefEle} data-raw={raw} className="clickable-sign">{error ? _raw.replace(regNumber, '-') : (data ? _raw.replace(regNumber, data[0].hit) : _raw)}</span>
        <DetailPannel ref={setPopper} data={data} style={styles.popper} show={showPannel} />
    </>
}
